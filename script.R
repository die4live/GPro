...
## AGI ref from Affy
{
    ...
    ### set class
    #ref.AGI.todo[, 2:17] <- as.character(ref.AGI.todo[, 2:17])
}

# matchup AGI between 2011MSB & Affy
matchup <- function(seq.data = seq.AGI, ref.data = ref.AGI.todo, seq.col = 1) {
    matchup.out <- data.frame()
    for (i in c(1:dim(seq.data)[1])) {
        seq <- seq.data[i, seq.col]
        loc <- which(ref.data == seq)
        if (length(loc) != 0) {
            col <- loc %/% dim(ref.data)[1]
            row <- loc %% dim(ref.data)[1]
            matchup.row <- ref.data[row, ]
            matchup.out <- rbind(matchup.out, matchup.row)
        }
    }
    return(matchup.out)
}
